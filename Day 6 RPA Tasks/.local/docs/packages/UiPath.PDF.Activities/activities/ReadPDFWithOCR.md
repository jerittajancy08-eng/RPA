...
| `DegreeOfParallelism` | Degree of parallelism | `InArgument` | `int` | Yes | `1` | Number of pages to be OCRed in parallel. A value of `-1` indicates the operation will be executed on available LogicalProcessorCount - 1. A positive value n will execute the operation on n logical processors, no greater than LogicalProcessorCount - 1. |
