...
Exports a single page from a PDF file as an image. The page is rendered at the specified DPI and saved to the output path. Supported image formats are PNG, JPEG, BMP, and TIFF — the format is determined automatically from the output file extension.
...
| `OutputFileName` | Output file path | `InArgument` | `string` | Yes | | | Full path of the output image file. The file extension determines the image format (`.png`, `.jpg`/`.jpeg`, `.bmp`, `.tif`/`.tiff`). |
...
- The output image format is inferred from the `OutputFileName` extension. Supported extensions: `.png`, `.jpg`, `.jpeg`, `.bmp`, `.tif`, `.tiff`.
